...
function SearchCard(props) {
	...
	function sendFriendRequest(event) {
		Axios.post("http://35.195.94.48:8080/api/add-friend-request", {
			fromemail: userEmail,
			sessionid: userID,
			fromind: "0",
			toind: "0",
	...
	return (
		<Card className="p-5 m-3 ml-auto mr-auto" style={{ width: "70%" }}>
			<div className="mt-auto mb-auto ml-auto mr-auto">
				...
				/>
				<h5 className="text-center ml-auto mr-auto">{props.name}</h5>
			</div>
			<Button
...
export default function Search() {
	...
	function searchForUser(event) {
		Axios.get(
			"http://35.195.94.48:8080/api/get-users?fullname=" +
				searchValue +
				"&petname=" +
				userPetName
		).then((response) => {
		...
		<>
			...
			<Container>
				<Form onSubmit={searchForUser}>
					<Form.Group size="lg" className="mt-3 text-center ml-auto mr-auto">
